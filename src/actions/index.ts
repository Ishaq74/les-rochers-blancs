...
export const server = {
  ...
  // FORMATIONS
  // -------------------------------------------------
  listFormations: defineAction({
    handler: async (_input, ctx) => {
      adminGuard(ctx);
      const rows = await db.select().from(schema.formations).orderBy(asc(schema.formations.sortOrder));
      const translations = await db.select().from(schema.formationTranslations);
      return rows.map(r => ({
        ...r,
        translations: translations.filter(t => t.formationId === r.id),
      }));
    },
  }),

  saveFormation: defineAction({
    input: z.object({
      id: uuidField.optional(),
      slug: z.string().min(1),
      imageUrl: z.string().optional(),
    ...
    handler: async (input, ctx) => {
      ...
      return db.transaction(async (tx) => {
        ...
        if (id) {
          await tx.update(schema.formations).set({ ...data, updatedAt: new Date() }).where(eq(schema.formations.id, id));
          formationId = id;
          await tx.delete(schema.formationTranslations).where(eq(schema.formationTranslations.formationId, id));
        } else {
          const [row] = await tx.insert(schema.formations).values(data).returning({ id: schema.formations.id });
          formationId = row.id;
        }

        if (translations.length > 0) {
          await tx.insert(schema.formationTranslations).values(
            translations.map(t => ({ ...t, formationId }))
          );
        }
  ...
  deleteFormation: defineAction({
    input: z.object({ id: uuidField }),
    handler: async (input, ctx) => {
      adminGuard(ctx);
      await db.delete(schema.formations).where(eq(schema.formations.id, input.id));
      return { success: true };
    },
  }),
